...
<script type="text/javascript" >
    ...
    nextThursday = moment(moment(moment(moment().day(5).add(6, 'days')).hour(0)).minute(00)).second(00);
    ...
    $('#toUpdate').datepicker({
        constrainInput: true,
        onSelect: function (selectedDate) {
            var tempDate = $('#fromUpdate').val();
            $('#fromUpdate').datepicker("option", "maxDate", selectedDate);
            $('#fromUpdate').datepicker("option", "defaultDate", new Date(tempDate));
            $('#fromUpdate').val(new Date(tempDate));
        }
    })
    $('#timeNew').timepicker({
        hourMin: 8,
