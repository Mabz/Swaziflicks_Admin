...
function updateMovieShowRanges() {
    ...
    objStarts = new Date(moment($('#fromUpdate').val()));
    objEnds = new Date(moment($('#toUpdate').val()));
    objStartAdvertising = new Date(moment($('#startAdvertisingUpdate').val()));
...
function updateStartTime() {
    ...
    objEnds = new Date(momentObjStarts.add(duration, 'minutes'));
...
function loadMovie(results) {
    ...
    for (var i = 0; i < results.length; i++) {
        ...
        if (results[i].get('Ad') == true) {
            var objAd = "1"
        } else {var objAd = "0"}
        var objRuntime = (results[i].get('Runtime').replace('mins', '')).replace('min', '');
        movies.innerHTML += "<button style='margin: 5px' onclick='interestMovie(&quot;" + objId + "&quot;,&quot;" + objTitle + "&quot;, &quot;" + objAd + "&quot;, &quot;" + objRuntime + "&quot;)' class='btn btn-default' aria-label='Left Align' id='" + objId + "'>" + objTitle + " | <span class='glyphicon glyphicon-film' aria-hidden='true'></span></button>";
    }
    if (results.length == 0) {
...
// Focus on Pricing
function interestMovie(objId, objTitle, objAd, objDuration) {
...
function interestShowDay(objId, objStarts, objEnds, objStartAdvertising) {
    var objEnds = new Date(moment(moment(moment(moment().day(5).add(6, 'days')).hour(22)).minute(59)).second(59));

    $('#toUpdate').val(objEnds);
    $('#toUpdate').datepicker("option", "defaultDate", objEnds);
    $('#toUpdate').datepicker("option", "minDate", new Date(objStarts));
    $('#fromUpdate').datepicker("option", "defaultDate", new Date(objStarts));
    $('#fromUpdate').datepicker("option", "maxDate", objEnds);
...
function newMovieShowings() {
    objId = null;
    objPT = $('#saveMovieShowings').attr('name');
    objStarts = new Date(moment($('#fromNew').val()).add(2, 'hours'));
    objEnds = new Date(moment($('#toNew').val()));
    objStartAdvertising = new Date(moment($('#startAdvertisingNew').val()));
    saveShowDay(objPT, objId, objStarts, objEnds, objStartAdvertising)
}
